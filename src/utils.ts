export const cleanPhoneNumber = (phoneNumber: string, countryCode: string) => {
    let phone = removeSpecialChars(phoneNumber.replace(/\s+/g, '').replace(/\+/g, '')).trim();
...
export const internationalize = (phoneNumber: string, countryCode: string, withPlus: boolean = false) => {
    let plus = withPlus ? '+' : '';
...
export const getRegexString = (countryCode: string, prefixes: string[], length: number) => {
